...
public class CaptureHelper extends ScreenRecorder {
    ...
    public static void stopRecord(int delayFinishSecond) {
        try {
            // Chuyển đổi giây thành mili giây và chờ
            TimeUnit.SECONDS.sleep(delayFinishSecond);
            screenRecorder.stop();
    ...
    //Tạo format ngày giờ để gắn vô name của screenshot hoặc record video không bị trùng tên (không bị ghi đè file)
